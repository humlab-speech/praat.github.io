...
void Melder_flushError () {
	if (Melder_hasCrash ()) {
		...
		throw MelderError ();   // pladdrr: was abort() (CRAN forbids abort); propagate the crash to the R boundary
	} else {
		/*
...
[[noreturn]] void Melder_fatal_ (const MelderArg& arg1,
...
[[noreturn]] void Melder_assert_ (const char *pathName, int lineNumber, const char *condition) {
	/*
